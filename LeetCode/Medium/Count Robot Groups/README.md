...
| **Link** | [View Problem](https://leetcode.com/problems/count-robot-groups/) |
| **Runtime** | 3 ms |
| **Memory** | 202.7 MB |

## Approach

is the robot speed is greater than the speed of the next grp or diff in dis is less than the distance then it can be merged

## Problem Description

<p>You are given a <span data-keyword="strictly-increasing-array" class=" cursor-pointer relative text-dark-blue-s text-sm"><button type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-_r_3o_" data-state="closed" class="">strictly increasing</button></span> integer array <code>position</code>, where <code>position[i]</code> is the initial position of the <code>i<sup>th</sup></code> robot at time <code>t = 0</code>.</p>
